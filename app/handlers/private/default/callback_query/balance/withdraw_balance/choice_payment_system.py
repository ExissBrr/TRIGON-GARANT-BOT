...
from app.data.types.payment_data import PaymentType, PaymentSystemStatus
...
@dp.callback_query_handler(menu_profile_cd.filter(command=MenuProfileCD.withdraw_balance))
async def send_menu_choice_payment_system(call: CallbackQuery, user: User, lang_code):
    ...
    await call.message.answer(
        ...
        reply_markup=keyboards.default.inline.balance.payment_system_list.keyboard(PaymentType.OUT, payment_systems,
                                                                                   lang_code)
    )
