...
@dp.message_handler(state=MomentaryMessageSendingStates.wait_for_roles, text=proceed)
async def request_message(message: Message, state_data: dict, lang_code):
    selected_roles: str = state_data.get('roles', None)
    await MomentaryMessageSendingStates.wait_for_chats.set()
    await request_for_chat_id(message, lang_code, is_roles=selected_roles)


@dp.message_handler(state=MomentaryMessageSendingStates.wait_for_roles, text=skip)
async def request_message(message: Message, state_data: dict, lang_code):
    selected_roles: str = state_data.get('roles', None)
    await MomentaryMessageSendingStates.wait_for_chats.set()
